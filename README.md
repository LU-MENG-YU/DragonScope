...
這不是統計分析系統。沒有回歸、相關、模型、SHAP、任意 Dashboard Builder 或複雜客製化。看到值得追查的現象，再回到專門分析系統處理。
...
### 本地狀態總覽
- 未匯入時完全鎖定，不從公開網路猜測內部狀態。
- 匯入後自動辨識日期、球員、軍別、位置。
- 內建辨識 Weight / PBF / SMM / FFMI / EV / FF。
- **不限上述欄位**：其他可解析的數值欄位也會自動成為 metric。
...
初始公開快取刻意是空的，不放假資料。頁面會顯示「尚未首次更新」。
...
> 公開情報回答「發生了什麼、接下來是什麼」；本地狀態回答「現在整體看起來怎樣」。原因、關係與預測交給分析系統。
